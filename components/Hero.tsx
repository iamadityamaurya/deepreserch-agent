  ...
  return (
    <section className="relative pt-16 pb-12 md:pt-20 md:pb-16 max-w-4xl mx-auto px-4 text-center space-y-8">
      ...
      <div className="space-y-3">
        <div className="inline-flex items-center space-x-1.5 px-3 py-1 rounded-full bg-indigo-950/60 border border-indigo-500/30 text-xs text-indigo-300 font-medium">
      ...
      <div className="max-w-2xl mx-auto space-y-4">
        <div className="relative flex items-center p-1.5 rounded-2xl bg-slate-900 border border-slate-800 focus-within:border-indigo-500/60 focus-within:ring-2 focus-within:ring-indigo-500/20 shadow-xl transition-all">
          <Search className="w-5 h-5 text-indigo-400 ml-3.5 mr-2 flex-shrink-0" />
          <input
            type="text"
            value={topic}
            ...
            disabled={isLoading}
            className="w-full py-3 bg-transparent text-slate-100 placeholder-slate-500 focus:outline-none text-sm sm:text-base"
          />
          <button
        ...
        <div className="flex flex-wrap items-center justify-center gap-2 pt-1">
          {samplePrompts.map((p, idx) => (
            <button
              ...
              className="px-3 py-1.5 rounded-lg bg-slate-900/90 hover:bg-slate-800 border border-slate-800 hover:border-indigo-500/40 text-xs text-slate-300 hover:text-indigo-200 transition-all cursor-pointer"
            >
              {p.label}
            </button>
          ))}
      ...
      <div className="pt-6 border-t border-slate-800/60 flex flex-wrap items-center justify-center gap-4 text-xs">
        <span className="font-medium text-slate-500">Integrated Sources:</span>
        {toolsList.map((t, idx) => (
          <div key={idx} className="flex items-center space-x-1.5 text-slate-300 font-medium">
            {t.icon}
            <span>{t.label}</span>
          </div>
        ))}
